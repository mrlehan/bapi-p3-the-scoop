...
//Need to add functions below for the object properties added for comments above
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.username && requestComment.articleId
    && requestComment.body && database.users[requestComment.username]
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const commentArticle = database.articles[savedComment.articleId].commentIds;
    commentArticle.splice(commentArticle.indexOf(id),1);

    const commentUser = database.users[savedComment.username].commentIds;
    commentUser.splice(commentUser.indexOf(id),1);
...
//retrieves id for comment to delete
  const id = Number(url.split('/').filter(segment => segment)[1]);
//retrieves the article object for the id provided
  const savedArticle = database.articles[id];
//establishes shell for returned response within the function
  const response = {};

  if (savedArticle) {
    //if there is a deleted article provided, then delete the article
    database.articles[id] = null;

    //for each Comment Id for the specific article, do the following
    savedArticle.commentIds.forEach(commentId => {
      //removes the comments from the comments table
      const comment = database.comments[commentId];
      database.comments[commentId] = null;
      //removes the comment from the user table
      const userCommentIds = database.users[comment.username].commentIds;
      userCommentIds.splice(userCommentIds.indexOf(id), 1);
    });
    //removes the article from the user.
